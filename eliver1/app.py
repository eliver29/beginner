...
def get_gender_distribution():
    ...
    gender_distribution = df_licensed.groupby('Gender')['Licensed'].sum()
    return {
        "labels": gender_distribution.index.tolist(),
        "values": gender_distribution.tolist()
    }
...
def get_top_5_vehicle_types_brands():
    _, _, df_violations = load_data()
    df_filtered = df_violations[(df_violations["Year"] >= 2020) & (df_violations["Year"] <= 2024)]
    top_5 = df_filtered.groupby(["VehicleType", "Brand"]).size().nlargest(5).reset_index()
    top_5.columns = ["VehicleType", "Brand", "Count"]
    return {"labels": [f"{row['VehicleType']} - {row['Brand']}" for _, row in top_5.iterrows()], "values": top_5["Count"].tolist()}
...
@app.route('/top10_common_violations')
def top10_vehicle_types_brands():
    return jsonify(get_top_5_vehicle_types_brands())

@app.route('/top5_vehicle_types_brands')
def top5_vehicle_types_brands():
    return jsonify(get_top_5_vehicle_types_brands())
